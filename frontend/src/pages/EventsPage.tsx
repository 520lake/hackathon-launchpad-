...
import HackathonCard, { type HackathonCardData } from '../components/HackathonCardV2'
...
const parseAwards = (awardsDetail?: string) => {
  ...
  try {
    ...
    if (Array.isArray(awards) && awards.length > 0) {
      // 计算总奖金
      const totalCash = awards.reduce((sum, award) => sum + (award.amount || 0), 0)
      if (totalCash >= 10000) {
...
const MOCK_DATA: HackathonCardData[] = [
  {
    ...
    description: '探索人工智能的无限可能，与全球开发者一起创造未来。我们寻找最具创新性的 AI 应用解决方案。',
  ...
  },
  {
    id: '2',
    title: 'Web3 开发者挑战赛',
    description: '聚焦区块链、DeFi、NFT 等前沿技术，连接开发者与资本，打造下一代 Web3 应用。',
...
export default function EventsPage() {
  ...
  const transformToCardData = (hackathon: Hackathon): HackathonCardData => {
    ...
    return {
      ...
      tags: hackathon.theme_tags?.split(',').map(t => t.trim()).filter(Boolean).slice(0, 3) || [],
  ...
  const allLocations = useMemo(() => Array.from(new Set(
    hackathons.map(h => h.location).filter(Boolean) as string[]
  )), [hackathons])
  ...
  const filteredHackathons = useMemo(() => {
    return hackathons.filter(h => {
      ...
      const matchesStatus = statusFilters.length === 0 || statusFilters.includes(h.status)
      const matchesType = typeFilters.length === 0 || typeFilters.includes(h.format || 'offline')
      ...
      return matchesSearch && matchesStatus && matchesType && matchesLocation && matchesTag
    })
  }, [hackathons, searchQuery, statusFilters, typeFilters, locationFilter, tagFilter])
  ...
  // 状态选项
  const statusOptions = [
    { value: 'registration', label: '报名中', bgColor: 'bg-amber-500', textColor: 'text-amber-400', borderColor: 'border-amber-500', bgLight: 'bg-amber-500/10' },
    { value: 'upcoming', label: '期待开始', bgColor: 'bg-blue-500', textColor: 'text-blue-400', borderColor: 'border-blue-500', bgLight: 'bg-blue-500/10' },
    { value: 'ongoing', label: '进行中', bgColor: 'bg-emerald-500', textColor: 'text-emerald-400', borderColor: 'border-emerald-500', bgLight: 'bg-emerald-500/10' },
    { value: 'ended', label: '已结束', bgColor: 'bg-gray-500', textColor: 'text-gray-400', borderColor: 'border-gray-500', bgLight: 'bg-gray-500/10' },
    { value: 'published', label: '已发布', bgColor: 'bg-purple-500', textColor: 'text-purple-400', borderColor: 'border-purple-500', bgLight: 'bg-purple-500/10' }
  ]
      ...
      <div className="flex gap-10 px-8 pb-12">
        ...
        <div className="w-64 flex-shrink-0">
          <div className="sticky top-6">
            ...
            {/* 状态筛选 */}
            <div className="mb-6">
              <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">状态</h3>
            ...
            {/* 活动类型筛选 */}
            <div className="mb-6">
              <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">活动类型</h3>
              <div className="space-y-2">
                {['线下', '线上', '混合'].map(type => (
            ...
            {/* 地点下拉 */}
            <div className="mb-6">
              <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">地点</h3>
            ...
            {/* 标签下拉 */}
            <div>
              <h3 className="text-xs text-gray-500 uppercase tracking-wider mb-3">标签</h3>
              <div className="relative">
                <select
                  value={tagFilter}
        ...
        <div className="flex-1 min-w-0">
          ...
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map(i => (
                <div key={i} className="bg-white/[0.03] border border-white/[0.08] rounded-3xl p-5 animate-pulse">
