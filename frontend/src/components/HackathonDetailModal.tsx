...
export default function HackathonDetailModal({
  ...
  const [contact, setContact] = useState<
    { text?: string; image?: string } | any[] | null
  >(null);
  ...
  // Mark occasionally unused reactive values as read so TypeScript doesn't flag them,
  // while keeping the logic available for future UI use.
  void scoring;
  void aiAnalysis;
  void isAnalyzing;
  void isPostsLoading;
  ...
  void loadingRecruitments;
  void setLoadingRecruitments;
  void setRecruitmentSearch;
  void setSkillFilter;
  ...
  const fetchRecruitments = async () => {
    try {
      ...
      const res = await axios.get(
        `${API_BASE_URL}/api/v1/teams/recruitments/all?hackathon_id=${hackathonId}`,
        {
  ...
  const fetchInsights = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.get(
        `/api/v1/community/insights?hackathon_id=${hackathonId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleRecruitSubmit = async () => {
    ...
    try {
      ...
      const API_BASE_URL =
        import.meta.env.VITE_API_URL || "http://localhost:8000";
      await axios.post(
        `${API_BASE_URL}/api/v1/teams/${myTeam.id}/recruitments`,
        recruitForm,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const LockedView = () => (
    <div className="flex flex-col items-center justify-center py-20 text-center h-full">
      ...
      <h3 className="text-2xl font-bold text-white mb-2">
        {lang === "zh" ? "请先报名" : "Registration Required"}
      </h3>
      <p className="text-gray-400 mb-6 max-w-md">
        {lang === "zh"
          ? "您需要先报名参加活动，才能查看此内容。"
          : "You need to register for the hackathon to view this content."}
      </p>
      {renderActionButton()}
    </div>
  );
  ...
  // Ensure analysis helpers and community handlers are treated as used
  void handleAnalyzeParticipants;
  ...
  const fetchHackathon = async () => {
    ...
    try {
      ...
      try {
        if (h.awards_detail)
          setAwards(
            typeof h.awards_detail === "string"
              ? JSON.parse(h.awards_detail)
              : h.awards_detail,
          );
      } catch (e) {
        setAwards([]);
      ...
      try {
        if (h.contact_info)
          setContact(
            typeof h.contact_info === "string"
              ? JSON.parse(h.contact_info)
              : h.contact_info,
          );
      } catch (e) {
        setContact(null);
      ...
      try {
        if (h.scoring_dimensions)
          setScoring(
            typeof h.scoring_dimensions === "string"
              ? JSON.parse(h.scoring_dimensions)
              : h.scoring_dimensions,
          );
      } catch (e) {
        setScoring([]);
      }
      try {
        if (h.results_detail)
          setResults(
            typeof h.results_detail === "string"
              ? JSON.parse(h.results_detail)
              : h.results_detail,
          );
      } catch (e) {
        setResults([]);
      ...
      try {
        if (h.sponsors_detail)
          setSponsors(
            typeof h.sponsors_detail === "string"
              ? JSON.parse(h.sponsors_detail)
              : h.sponsors_detail,
          );
      } catch (e) {
        setSponsors([]);
      ...
      if (token) {
        try {
          ...
          const myEnroll = resEnroll.data.find(
            (e: any) => Number(e.hackathon_id) === Number(hackathonId),
          );
          ...
          // My Team
          const resTeams = await axios.get("/api/v1/teams/me", {
            headers: { Authorization: `Bearer ${token}` },
          });
          const myTeamFound = resTeams.data.find(
            (t: any) => Number(t.hackathon_id) === Number(hackathonId),
          );
          ...
          if (myTeamFound) {
            ...
            const myProj = resProj.data.find(
              (p: any) => Number(p.team_id) === Number(myTeamFound.id),
            );
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post(
        "/api/v1/enrollments/",
        { hackathon_id: hackathonId, user_id: 0 },
        {
      ...
      alert(
        lang === "zh"
          ? "🎉 报名成功！已为您解锁“我的项目”空间。"
          : '🎉 Registration Successful! "My Project" space is now unlocked.',
      );
    } catch (e: any) {
      ...
      if (
        ...
        alert(
          lang === "zh"
            ? "您已报名，正在跳转..."
            : "You are already enrolled. Redirecting...",
        );
        return;
      }
  ...
  const handleLeaveTeam = async () => {
    ...
    if (myTeam.leader_id === currentUserId) {
      alert(
        lang === "zh"
          ? "作为队长，您无法直接退出战队。"
          : "As the leader, you cannot leave the team.",
      );
      return;
    }
    if (
  ...
  const handleStartIndividual = async () => {
    ...
    try {
      ...
      await axios.post(
        "/api/v1/teams",
        {
          name: `${lang === "zh" ? "个人项目" : "Individual Project"} - ${currentUserId}`,
        },
        {
          params: { hackathon_id: hackathonId },
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
  ...
  const renderActionButton = () => {
    ...
    const regStart = hackathon.registration_start_date
      ? new Date(hackathon.registration_start_date).getTime()
      : 0;
    const regEnd = hackathon.registration_end_date
      ? new Date(hackathon.registration_end_date).getTime()
      : 0;
    const actStart = hackathon.start_date
      ? new Date(hackathon.start_date).getTime()
      : 0;
    const actEnd = hackathon.end_date
    ...
    if (currentUserId && (enrollment || myTeam)) {
      return (
        <button
          ...
          className="btn-primary w-full md:w-auto text-xl px-8 py-3 hover:opacity-90 active:scale-95 transition-all shadow-lg hover:shadow-brand/50"
        >
          {lang === "zh" ? "进入社区" : "ENTER COMMUNITY"}
        </button>
      );
    }
    ...
    if (now < regStart) {
      return (
        <button disabled className="btn-disabled w-full md:w-auto">
          {lang === "zh" ? "报名未开始" : "NOT STARTED"}
        </button>
      );
    }
    ...
    if (now >= regStart && now < regEnd) {
      ...
      return (
        <button
          ...
          className="btn-primary w-full md:w-auto text-xl px-8 py-3 hover:opacity-90 active:scale-95 transition-all shadow-lg hover:shadow-brand/50 flex items-center justify-center gap-2"
        >
          {registerLoading && (
  ...
  const fetchPosts = async () => {
    ...
    try {
      const res = await axios.get(
        `/api/v1/community/posts?hackathon_id=${hackathonId}`,
      );
      setPosts(res.data);
    } catch (e) {
      console.error(e);
  ...
  const handleGenerateTopic = async () => {
    try {
      await axios.post(
        `/api/v1/community/generate?hackathon_id=${hackathonId}`,
        {},
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        },
  ...
  // Dynamic Filters
  const availableRoles = Array.from(new Set(recruitments.map((r) => r.role)))
    .filter(Boolean)
    .sort();
  const availableSkills = Array.from(
    new Set(
      recruitments.flatMap((r) =>
  ...
  const filteredRecruitments = recruitments.filter((r) => {
    ...
    const matchesRole =
      !roleFilter || r.role.toLowerCase().includes(roleFilter.toLowerCase());
    const matchesSkill =
      !skillFilter ||
      r.skills.toLowerCase().includes(skillFilter.toLowerCase());

    return matchesSearch && matchesRole && matchesSkill;
  });

  // Keep community helpers and derived filters marked as used for TypeScript,
  // even if some UI paths are not currently wired up.
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 backdrop-blur-md p-0">
      ...
      <div
        ref={containerRef}
        className="bg-surface w-full h-full flex flex-col relative border-none shadow-none overflow-hidden"
      >
        {loading || !hackathon ? (
          ...
          <div
            className="h-full overflow-y-auto scrollbar-thin relative bg-surface"
          ...
          >
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-full bg-brand/20 border border-brand/50 flex items-center justify-center text-brand font-bold">
                      {hackathon.organizer_name
                        ? hackathon.organizer_name[0].toUpperCase()
                        : "O"}
                    </div>
                    <div>
                      <div className="text-[10px] text-gray-400 uppercase tracking-widest mb-0.5">
                        {lang === "zh" ? "主办方" : "ORGANIZED BY"}
                      </div>
                      <div className="font-bold text-white hover:text-brand cursor-pointer transition-colors text-sm">
                        {hackathon.organizer_name || "Unknown"}
                      </div>
                    </div>
                  </div>
            ...
            <div className="sticky top-0 z-40 flex border-b border-brand/20 bg-black/80 backdrop-blur-md overflow-x-auto">
              {[
                {
                  id: "overview",
                  label: lang === "zh" ? "活动详情" : "OVERVIEW",
                },
                {
                  id: "my_project",
                  label: lang === "zh" ? "我的项目" : "MY PROJECT",
                },
                {
                  id: "participants",
                  label: lang === "zh" ? "社区 & 组队" : "COMMUNITY",
                },
                {
                  id: "gallery",
                  label: lang === "zh" ? "项目展示" : "GALLERY",
                },
                {
                  id: "results",
                  label: lang === "zh" ? "评审结果" : "RESULTS",
                },
                ...(enrollment?.status === "approved"
                  ? [
                      {
                        id: "tools",
                        label: lang === "zh" ? "AI 工具箱" : "AI TOOLS",
                      },
                    ]
                  : []),
              ].map((tab) => (
                      ...
                      <section id="schedule" className="scroll-mt-24">
                        ...
                        <div className="bg-white/5 border border-white/10 p-8 rounded-sm space-y-6 hover:border-brand/30 transition-colors">
                          ...
                          <div className="flex justify-between items-center pb-2">
                            <span className="text-brand font-mono text-sm uppercase tracking-wider">
                              {lang === "zh"
                                ? "作品提交截止"
                                : "SUBMISSION DEADLINE"}
                            </span>
                            <span className="text-brand font-mono text-xl font-bold">
                        ...
                        <section id="judges" className="scroll-mt-24">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {judges.map((judge) => (
                              <div
                                key={judge.id}
                                className="bg-white/5 border border-white/10 p-6 flex items-start gap-4 hover:border-brand/30 transition-colors"
                              >
                                <div className="w-16 h-16 shrink-0 bg-brand/20 rounded-full overflow-hidden border border-brand/20">
                                  {judge.avatar_url ? (
                                    <img
                                      src={judge.avatar_url}
                                      alt={judge.nickname || judge.full_name}
                                ...
                                <div>
                                  <div className="text-white font-bold text-lg mb-1">
                                    {judge.nickname || judge.full_name}
                                  </div>
                                  {judge.bio && (
                        ...
                        <section id="awards">
                          ...
                          <div className="grid grid-cols-1 gap-6">
                            {awards.map((award, i) => (
                              <div
                                key={i}
                                className="relative overflow-hidden bg-gradient-to-r from-brand/5 to-transparent border border-brand/20 p-6 flex flex-col md:flex-row justify-between items-center group hover:border-brand/50 transition-all"
                              >
                                ...
                                <div className="mb-4 md:mb-0 text-center md:text-left">
                                  <div className="text-brand font-black text-xl uppercase tracking-wider mb-1">
                                    {award.name}
                                  </div>
                                ...
                                <div className="text-3xl font-black text-white tracking-tighter text-shadow-brand">
                                  {award.prize}
                                </div>
                              </div>
                            ))}
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {sponsors.map((sponsor, idx) => (
                              <a
                                ...
                                className="block bg-white/5 border border-white/10 p-4 hover:border-brand/50 transition-all group"
                              >
                                ...
                                <div className="text-center">
                                  <div className="text-sm font-bold text-gray-300 group-hover:text-brand truncate">
                                    {sponsor.name}
                                  </div>
                                </div>
                              </a>
                    ...
                    <div className="hidden md:block col-span-1 space-y-6">
                      ...
                      {(enrollment || myTeam) && (
                        <div className="bg-brand/10 border border-brand/50 p-6 relative overflow-hidden group">
                          <div className="absolute top-0 right-0 p-2 opacity-10 text-6xl group-hover:opacity-20 transition-opacity">
                            ⚡
                          </div>
                          <h4 className="text-brand font-black uppercase mb-2">
                            {lang === "zh" ? "寻找队友" : "TEAM MATCH"}
                          </h4>
                              ...
                              <>
                                {!Array.isArray(contact) ? (
                                  ...
                                  contact.map((item: any, idx: number) => (
                                    ...
                                    >
                                      {item.type && (
                                        <span className="text-brand font-mono text-xs mr-2 uppercase">
                                          {item.type}:
                                        </span>
                                      )}
                                      {item.type === "image" ||
                                      item.type === "qr" ? (
                                        <img
                                          src={item.value}
                                          alt="Contact"
                                          className="mt-1 w-full h-auto rounded border border-white/10"
                                        />
                                      ) : (
                ...
                {activeTab === "my_project" &&
                  (loading ? (
                    <div className="flex flex-col items-center justify-center py-40">
                      ...
                      <div className="text-brand font-mono animate-pulse">
                        LOADING DATA...
                      </div>
                    </div>
                  ) : !currentUserId || (!enrollment && !myTeam) ? (
                    <LockedView />
                  ) : (
                      ...
                      {!enrollment && !myTeam ? (
                        <div className="text-center py-20 border border-brand/20 bg-white/5">
                          <h3 className="text-2xl font-bold text-white mb-4">
                            {lang === "zh" ? "尚未报名" : "NOT ENROLLED"}
                          </h3>
                          <p className="text-gray-400 mb-8">
                            {lang === "zh"
                              ? "请先报名参加活动，然后再创建项目。"
                              : "Please register for the hackathon first."}
                          </p>
                          <button
                            onClick={handleRegister}
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                              ...
                              <div className="p-6 border border-brand/20 bg-white/5">
                                <h3 className="text-xl font-bold text-white mb-6">
                                  {lang === "zh" ? "创建战队" : "CREATE TEAM"}
                                </h3>
                                <div className="space-y-4">
                                  <input
                                    type="text"
                                    placeholder={
                                      lang === "zh" ? "战队名称" : "Team Name"
                                    }
                              ...
                              <div className="p-6 border border-white/10 bg-white/5">
                                <h3 className="text-xl font-bold text-white mb-6">
                                  {lang === "zh" ? "加入战队" : "JOIN TEAM"}
                                </h3>
                                <p className="text-gray-400 text-sm mb-4 h-20">
                                  {lang === "zh"
                                    ? "请在“参赛人员”页面找到想加入的战队，并联系队长。"
                                    : 'Please find a team in "Participants" tab and contact the leader.'}
                                </p>
                                <button
                                  onClick={() => setActiveTab("participants")}
                                  className="btn-secondary w-full py-3"
                                >
                                  {lang === "zh" ? "浏览战队" : "BROWSE TEAMS"}
                                </button>
                              </div>
                            </div>
                          )}
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="p-6 border border-brand/30 bg-brand/5 flex justify-between items-center">
                            <div>
                              <div className="text-xs text-brand font-mono uppercase tracking-widest mb-1">
                                {lang === "zh" ? "我的战队" : "MY TEAM"}
                              </div>
                            ...
                            <div className="text-right">
                              ...
                              <button
                                ...
                                className="text-xs text-red-500 hover:text-red-400 font-mono uppercase underline"
                              >
                          ...
                          <div className="p-6 border border-white/10 bg-white/5">
                            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">
                              {lang === "zh" ? "战队成员" : "TEAM MEMBERS"}
                            </h3>
                            <div className="flex flex-wrap gap-4">
                              {myTeam.members?.map((member) => (
                                <div
                                  key={member.id}
                                  className="flex items-center gap-3 bg-black/40 border border-white/10 px-4 py-2 rounded-full"
                                >
                            ...
                            <div className="mt-6 pt-6 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
                              <div>
                                ...
                                <p className="text-xs text-gray-400">
                                  {lang === "zh"
                                    ? "邀请更多伙伴加入你的战队"
                                    : "Invite more members to join your team"}
                                </p>
                              </div>
                                ...
                                <button
                                  onClick={() => {
                                    navigator.clipboard.writeText(
                                      `${window.location.origin}/hackathons/${hackathon.id}?invite=${myTeam.id}`,
                                    );
                                    alert(
                                      lang === "zh"
                                        ? "邀请链接已复制"
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-2">
                                      {myProject.tech_stack
                                        ...
                                        .map((s, i) => (
                                          <span
                                            key={i}
                                            className="px-2 py-1 bg-brand/10 border border-brand/30 text-brand text-xs font-mono uppercase"
                                          >
                                            {s.trim()}
                                          </span>
                                        ))}
                            ...
                            <AIProjectAssistant
                              ...
                              onIdeaSelect={(idea) => {
                                ...
                                setInitialProjectData({
                                  ...
                                  tech_stack: Array.isArray(idea.tech_stack)
                    ...
                    <div className="w-72 flex-shrink-0">
                      <div className="sticky top-24 space-y-1">
                        ...
                        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.08]">
                          <span className="text-[11px] font-medium tracking-[0.1em] text-gray-400 uppercase">
                            筛选条件
                          </span>
                          <button
                            onClick={() => {
                        ...
                        <div className="px-4 py-4 border-b border-white/[0.05]">
                          <div className="text-[10px] font-medium tracking-[0.15em] text-gray-600 uppercase mb-3">
                            身份类型
                          </div>
                          <div className="space-y-1">
                            <label className="flex items-center gap-3 cursor-pointer group py-1.5 px-2 -mx-2 hover:bg-white/[0.03] transition-colors rounded">
                              <div
                                className={`w-4 h-4 border ${identityFilter === "individual" || identityFilter === "all" ? "bg-brand border-brand" : "border-white/20"} flex items-center justify-center transition-all`}
                              >
                                {(identityFilter === "individual" ||
                                  identityFilter === "all") && (
                              ...
                              <span className="text-[13px] text-gray-300 group-hover:text-white font-light">
                                个人
                              </span>
                            ...
                            <label className="flex items-center gap-3 cursor-pointer group py-1.5 px-2 -mx-2 hover:bg-white/[0.03] transition-colors rounded">
                              <div
                                className={`w-4 h-4 border ${identityFilter === "team" || identityFilter === "all" ? "bg-brand border-brand" : "border-white/20"} flex items-center justify-center transition-all`}
                              >
                                {(identityFilter === "team" ||
                                  identityFilter === "all") && (
                                  <span className="text-black text-[10px]">
                        ...
                        <div className="px-4 py-4 border-b border-white/[0.05]">
                          ...
                          <div className="space-y-1">
                            {[
                              ...
                              <label
                                key={role.key}
                                className="flex items-center gap-3 cursor-pointer group py-1.5 px-2 -mx-2 hover:bg-white/[0.03] transition-colors rounded"
                              >
                                <div
                                  className={`w-4 h-4 border ${roleFilter === role.key ? "bg-brand border-brand" : "border-white/20"} flex items-center justify-center transition-all`}
                                >
                                  {roleFilter === role.key && (
                                    <span className="text-black text-[10px]">
                                      ✓
                        ...
                        <div className="px-4 py-4 border-b border-white/[0.05]">
                          ...
                          <div className="space-y-1">
                            {[
                              ...
                              >
                                <div
                                  className={`w-4 h-4 border ${statusFilter === status.key || statusFilter === "all" ? "bg-brand border-brand" : "border-white/20"} flex items-center justify-center transition-all`}
                                >
                                ...
                                <span
                                  className={`text-[13px] ${status.color} group-hover:text-white font-light`}
                                >
                                  {status.label}
                                </span>
                                <span className="text-[11px] text-gray-600 ml-auto font-mono">
                              ...
                              <div className="flex-1 p-5">
                                <div className="flex gap-5">
                                  ...
                                  <div className="flex-1 min-w-0 py-1">
                                    <div className="flex items-center gap-3 mb-2">
                                      <h4 className="text-[15px] font-semibold text-white group-hover:text-brand transition-colors tracking-tight">
                                        {team.name}
                                      </h4>
                                      <div className="flex items-center gap-2">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        {team.recruitments
                                          ...
                                          .map((r) => (
                                            <span
                                              key={r.id}
                                              className="px-2 py-0.5 text-[11px] border border-white/[0.08] text-gray-400 hover:border-brand/30 hover:text-gray-300 transition-colors font-light"
                                            >
                                              {r.role}
                                            </span>
                                          ))}
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {(p.skills || [])
                                        ...
                                        .map((s: string, i: number) => (
                                          <span
                                            key={i}
                                            className="px-2 py-0.5 text-[11px] border border-white/[0.08] text-gray-400 font-light"
                                          >
                                            {s}
                                          </span>
                                        ))}
                ...
                {activeTab === "gallery" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {galleryProjects.map((proj) => (
                      <div
                        key={proj.id}
                        className="border border-white/10 bg-white/5 hover:border-brand/50 transition-colors group overflow-hidden flex flex-col h-full"
                      >
                        <div className="h-48 bg-black/50 relative overflow-hidden">
                          {proj.cover_image ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center text-white/10 text-4xl font-black bg-noise">
                              {proj.title[0]}
                            </div>
                          )}
                        </div>
                        <div className="p-6 flex-1 flex flex-col">
                          <h4 className="font-bold text-white text-lg mb-2 group-hover:text-brand transition-colors">
                            {proj.title}
                          </h4>
                          ...
                          <div className="flex justify-between items-center text-xs font-mono text-gray-500 pt-4 border-t border-white/5">
                            <span>
                              {proj.team?.name || `Team #${proj.team_id}`}
                            </span>
                            {proj.total_score ? (
                              <span className="text-brand font-bold">
                                {proj.total_score.toFixed(1)} PTS
                              </span>
                            ) : (
                              <span>PENDING</span>
                            )}
                    ...
                    {results.length > 0 ? (
                      <div className="space-y-6">
                        {results.map((result: any, idx: number) => (
                          <div
                            key={idx}
                            className="flex items-center gap-6 p-6 border border-brand/20 bg-brand/5 relative overflow-hidden"
                          >
                            <div className="text-6xl font-black text-white/10 absolute -left-4 -bottom-4 select-none">
                              {idx + 1}
                            </div>
                            ...
                            <div className="flex-1 relative z-10">
                              <h4 className="text-2xl font-bold text-white mb-1">
                                {result.project_title || result.name}
                              </h4>
