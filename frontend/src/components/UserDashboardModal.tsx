...
export default function UserDashboardModal({
  ...
  onTeamMatchClick: _onTeamMatchClick,
  ...
  const [activeTab, setActiveTab] = useState<
    "created" | "joined" | "projects" | "profile"
  >("created");
      ...
      >
        ...
        <div className="w-full md:w-72 bg-void/30 border-b md:border-b-0 md:border-r border-brand/20 flex flex-col relative z-10">
          <div className="p-6 border-b border-brand/20">
            ...
            {user && (
              <div className="mt-6 flex items-center gap-3">
                ...
                <div>
                  <div className="text-white font-bold font-mono text-sm truncate max-w-[140px]">
          ...
          >
            {[
              ...
              >
                ...
                {activeTab === tab.id && (
                  <div className="absolute inset-0 bg-brand/5 z-0 animate-pulse"></div>
                )}
              </button>
            ))}
          </div>
          ...
          <div className="p-4 border-t border-brand/20">
            <button
              onClick={onClose}
              className="w-full py-3 border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:border-red-500 transition-all font-mono text-sm uppercase tracking-wider"
            >
              关闭系统
            </button>
          </div>
        ...
        <div className="flex-1 bg-surface relative overflow-hidden flex flex-col">
          ...
          <div
            className="flex-1 overflow-y-auto p-6 md:p-10 relative z-10 custom-scrollbar"
            ref={contentRef}
          >
            {/* PROFILE TAB */}
            {activeTab === "profile" && (
              <div className="space-y-8 max-w-3xl mx-auto pb-10">
                ...
                <div className="group">
                  <label className="block text-xs font-bold text-brand mb-3 uppercase tracking-wider font-mono">
                    专业技能
                  </label>
                  ...
                  myCreated.map((h) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start relative z-10">
                        <div>
                          <h3 className="font-bold text-xl text-white font-mono group-hover:text-brand transition-colors">
                            {h.title}
                          </h3>
            ...
            {activeTab === "joined" && (
              <div className="space-y-8">
                {myJoined.length > 0 &&
                  ["registering", "upcoming", "ongoing", "ended", "other"].map(
                    (status) => {
                      const filtered = myJoined.filter((e) => {
                        ...
                        const actStart = h.start_date
                          ? new Date(h.start_date).getTime()
                          : 0;
                        const actEnd = h.end_date
                          ? new Date(h.end_date).getTime()
                          : 0;

                        // Schedule dates (registration_start/end) are now in the
                        // schedules table and not on the hackathon model. Use the
                        // overall start_date / end_date for basic filtering.
                        if (status === "registering")
                          return h.status === "published" && now < actStart;
                        if (status === "upcoming") return now < actStart;
                        if (status === "ongoing")
                          ...
                          {filtered.map((e) => (
                            ...
                            >
                              ...
                              <div className="flex justify-between items-start relative z-10">
                                <div>
                                  <h3 className="font-bold text-xl text-white font-mono group-hover:text-brand transition-colors">
                                    {e.hackathon?.title || "Unknown Hackathon"}
                                  </h3>
                                  <div className="flex items-center gap-4 mt-3">
                                    <p className="text-xs text-gray-500 font-mono uppercase">
                                      状态:{" "}
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start relative z-10">
                        <div>
                          ...
                          <p className="text-sm text-gray-400 mt-2 line-clamp-2 max-w-xl">
                            {p.description}
                          </p>
