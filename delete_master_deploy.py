...
CLEAN_DIR = r"d:\aurathon_clean_master"

print("🗑️  删除并重建 master 分支...")
...
# 提交
print("💾 提交...")
subprocess.run('git commit -m "Deploy: Aurathon with community and rounded UI"', 
...
# 删除远程 master 分支
print("\n🗑️  删除远程 master 分支...")
result = subprocess.run('git push origin --delete master', 
                       shell=True, capture_output=True, text=True)
if result.returncode == 0:
    print("  ✅ master 分支已删除")
else:
    print(f"  ⚠️  删除失败：{result.stderr}")
...
if result.returncode == 0:
    ...
    print(f"\n❌ 推送失败：{result.stderr}")
    print("\n尝试推送到 release 分支...")
    result = subprocess.run('git push -u origin master:release', 
                           shell=True, capture_output=True, text=True)
    if result.returncode == 0:
        print("\n✅ 已推送到 release 分支！")
        print("请在魔搭网页上选择 release 分支部署")
