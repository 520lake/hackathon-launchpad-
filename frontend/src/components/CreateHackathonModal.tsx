...
export default function CreateHackathonModal({
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      if (content.professionalism_tags)
      ...
      if (
        ...
        ["individual", "team"].includes(content.registration_type)
      ) {
      ...
      if (content.contact_info_text) {
        setContactInfo((prev) => ({
          ...prev,
          text: content.contact_info_text,
        }));
      }

      if (content.awards_detail && Array.isArray(content.awards_detail)) {
        setAwards(content.awards_detail);
      }

      if (content.scoring_dimensions)
      ...
      if (!startDate && !isRefining) {
        ...
        const resultDay = new Date(end);
        resultDay.setDate(resultDay.getDate() + 2);
      ...
      setAiChatHistory([
        ...newHistory,
        {
          ...
          content: isRefining
            ...
            : lang === "zh"
              ? "已生成活动草案，请查看下方表单。"
              : "Draft generated, please check the form below.",
        },
      ]);
      setAiPrompt("");
    } catch (e) {
  ...
  const validateStep2 = () => {
    ...
    if (scoringDimensions.length > 0) {
      const totalWeight = scoringDimensions.reduce(
    ...
    try {
      ...
      const payload = {
        ...
        submission_start_date: submissionStartDate
          ? new Date(submissionStartDate).toISOString()
          : null,
        submission_end_date: submissionEndDate
          ? new Date(submissionEndDate).toISOString()
          : null,
        judging_start_date: judgingStartDate
          ? new Date(judgingStartDate).toISOString()
          : null,
        judging_end_date: judgingEndDate
          ? new Date(judgingEndDate).toISOString()
          : null,
      ...
      if (initialData && initialData.id) {
        ...
        res = await axios.patch(
          `/api/v1/hackathons/${initialData.id}`,
          payload,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
      ...
      >
        ...
        <div className="p-4 md:p-6 border-b border-brand/20 flex justify-between items-center bg-surface/50">
          <h2 className="text-lg md:text-2xl font-black text-ink uppercase tracking-tight flex items-center">
            ...
            <div className="ml-4 md:ml-8 flex items-center space-x-2 text-xs md:text-sm font-mono">
              <span
                className={`px-2 py-1 ${currentStep === 1 ? "bg-brand text-black" : "text-gray-500"}`}
              >
                01 INFO
              </span>
              <span className="text-gray-600">--</span>
              <span
                className={`px-2 py-1 ${currentStep === 2 ? "bg-brand text-black" : "text-gray-500"}`}
              >
                02 DETAILS
              </span>
            </div>
        ...
        >
          ...
          <div className="mb-8 relative group overflow-hidden">
            ...
            <div
              className={`border border-brand/40 bg-black/60 transition-all duration-300 ${showAiChat ? "shadow-[0_0_30px_rgba(212,163,115,0.15)]" : "hover:border-brand/70"}`}
            >
              {/* Header / Entry Point */}
              <div
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-10 h-10 rounded-none bg-brand text-black flex items-center justify-center text-xl font-bold shadow-lg ${aiLoading ? "animate-pulse" : ""}`}
                  >
                    ⚡
                  </div>
                  <div>
                    ...
                    <p className="text-gray-400 text-xs font-mono uppercase tracking-wider">
                      {lang === "zh"
                        ? "自然语言生成完整活动方案"
                        : "GENERATE FULL EVENT PROTOCOL VIA NLP"}
                    </p>
                  </div>
                </div>
                <button
                  className={`px-4 py-2 border border-brand/50 text-brand text-xs font-bold uppercase hover:bg-brand hover:text-black transition-all ${showAiChat ? "bg-brand/10" : ""}`}
                >
                  {showAiChat
                    ? lang === "zh"
                      ? "收起助手"
                  ...
                  {aiChatHistory.length > 0 ? (
                    <div className="mb-4 max-h-60 overflow-y-auto space-y-4 pr-2 scrollbar-thin scrollbar-thumb-brand/20">
                      {aiChatHistory.map((msg, idx) => (
                        ...
                        >
                          <div
                            className={`max-w-[90%] p-3 text-sm font-mono border backdrop-blur-sm ${
                              msg.role === "user"
                    ...
                    <div className="relative flex gap-0 bg-black">
                      <textarea
                        ...
                        placeholder={
                          lang === "zh"
                            ? "在此描述您的活动构想..."
                            : "Describe your event concept here..."
                        }
                        value={aiPrompt}
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-brand text-xs font-bold mb-2 uppercase tracking-widest">
                      {lang === "zh" ? "活动主题 (标题云)" : "THEME TAGS"} *
                    </label>
                    <input
                      className="w-full bg-black/50 border border-brand/30 p-4 text-white focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand/20 placeholder-gray-600 transition-all"
                      placeholder={
                        lang === "zh"
                          ? "Web3, AI, GameFi (用逗号分隔)"
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-brand text-xs font-bold uppercase tracking-widest">
                      {lang === "zh" ? "封面缩略图" : "COVER IMAGE"} *
                    </label>
                  ...
                  <div className="border-2 border-dashed border-brand/40 h-[220px] bg-black/40 backdrop-blur-sm flex flex-col items-center justify-center relative overflow-hidden group hover:border-brand/80 hover:bg-brand/5 transition-all">
                    {coverImage ? (
                      ...
                      <div className="text-center p-4">
                        <div className="text-4xl mb-2 text-brand/70 group-hover:text-brand transition-colors">
                          ⊕
                        </div>
                        <span className="text-brand/60 text-xs font-mono uppercase group-hover:text-brand transition-colors">
                          Upload 16:9 Image
                        </span>
                      </div>
                    )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-brand/30">
                <div>
                  <label className="block text-brand text-xs font-bold mb-2 uppercase tracking-widest">
                    {lang === "zh" ? "报名形式" : "REGISTRATION TYPE"} *
                  </label>
                  <div className="flex space-x-4">
                    <label
                      className={`flex items-center space-x-2 cursor-pointer p-3 border-2 ${registrationType === "individual" ? "border-brand bg-brand/10 shadow-[0_0_15px_rgba(212,163,115,0.2)]" : "border-white/10 hover:border-brand/50 hover:bg-brand/5"} flex-1 transition-all`}
                    >
                      <input
                        type="radio"
                      ...
                      >
                        👤 {lang === "zh" ? "个人报名" : "Individual"}
                      </span>
                    </label>
                    <label
                      className={`flex items-center space-x-2 cursor-pointer p-3 border-2 ${registrationType === "team" ? "border-brand bg-brand/10 shadow-[0_0_15px_rgba(212,163,115,0.2)]" : "border-white/10 hover:border-brand/50 hover:bg-brand/5"} flex-1 transition-all`}
                    >
                      <input
                        type="radio"
                ...
                <div>
                  ...
                  <div className="flex space-x-4">
                    <label
                      className={`flex items-center space-x-2 cursor-pointer p-3 border-2 ${format === "online" ? "border-brand bg-brand/10 shadow-[0_0_15px_rgba(212,163,115,0.2)]" : "border-white/10 hover:border-brand/50 hover:bg-brand/5"} flex-1 transition-all`}
                    >
                      <input
                        type="radio"
                    ...
                    <label
                      className={`flex items-center space-x-2 cursor-pointer p-3 border-2 ${format === "offline" ? "border-brand bg-brand/10 shadow-[0_0_15px_rgba(212,163,115,0.2)]" : "border-white/10 hover:border-brand/50 hover:bg-brand/5"} flex-1 transition-all`}
                    >
                      <input
                        type="radio"
            ...
            <div className="space-y-8">
              ...
              <div>
                <h3 className="text-brand font-mono text-lg mb-4 border-b-2 border-brand/30 pb-2 flex items-center gap-2">
                  <span className="text-brand/70">//</span> ORGANIZATION &
                  SCHEDULE
                </h3>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      <input
                        ...
                        placeholder={
                          lang === "zh" ? "微信号/邮箱/电话" : "WeChat/Email"
                        }
                        ...
                        <div className="relative border-2 border-dashed border-brand/40 w-full h-28 bg-black/40 backdrop-blur-sm flex items-center justify-center hover:border-brand/80 hover:bg-brand/5 transition-all group">
                          {contactInfo.image ? (
                            ...
                            <div className="text-center">
                              <span className="text-brand/70 text-2xl group-hover:text-brand transition-colors">
                                ⊕
                              </span>
                              <div className="text-[10px] text-brand/60 uppercase mt-1 group-hover:text-brand transition-colors">
                                {lang === "zh"
                                  ? "上传二维码/名片"
                                  : "UPLOAD QR/CARD"}
                              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-gradient-to-br from-black/60 to-brand/5 p-5 border border-brand/30 backdrop-blur-sm">
                  <div>
                    ...
                    <div className="space-y-2">
                      <input
                        type="date"
                        className="w-full bg-black/60 border border-brand/30 p-2.5 text-xs text-white focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand/20 transition-all"
              ...
              <div>
                <h3 className="text-brand font-mono text-lg mb-4 border-b-2 border-brand/30 pb-2 flex items-center gap-2">
                  <span className="text-brand/70">//</span> DETAILS &
                  REQUIREMENTS
                </h3>
                <div className="space-y-5">
                  <div>
                    <label className="block text-brand text-xs font-bold mb-2 uppercase tracking-widest">
                      {lang === "zh" ? "活动详情" : "DESCRIPTION"} *
                    </label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-brand text-xs font-bold mb-2 uppercase tracking-widest">
                      {lang === "zh" ? "资源与支持" : "RESOURCES & SUPPORT"}
                    </label>
                    <textarea
                      className="w-full h-24 bg-black/50 border border-brand/30 p-4 text-white focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand/20 placeholder-gray-600 font-mono text-sm transition-all"
                      placeholder={
              ...
              <div>
                ...
                {/* Awards List */}
                <div className="mb-6">
                  <label className="block text-brand text-xs font-bold mb-3 uppercase tracking-widest">
                    {lang === "zh" ? "奖项设置" : "AWARDS"}
                  </label>
                  <div className="space-y-2 mb-4">
                    {awards.map((award, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-2 bg-gradient-to-r from-brand/5 to-transparent p-3 border-l-2 border-brand"
                      >
                        ...
                        <button
                          ...
                          className="text-red-500 hover:text-red-400 hover:bg-red-500/10 px-2 py-1 rounded transition-colors"
                        >
                          ✕
                        </button>
                      </div>
                  ...
                  <div className="grid grid-cols-12 gap-2 bg-black/40 p-3 border border-brand/30 backdrop-blur-sm">
                    ...
                    {(newAward.type === "other" ||
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewAward({ ...newAward, prize: e.target.value })
                        }
                      />
                    )}
                    <button
                      onClick={addAward}
                      className="col-span-1 bg-brand text-black font-black hover:bg-white hover:scale-105 transition-all"
                    >
                      +
                    </button>
                  </div>
                ...
                <div className="mt-8 pt-6 border-t-2 border-brand/30">
                  <label className="block text-brand text-xs font-bold mb-4 uppercase tracking-widest">
                    {lang === "zh"
                      ? "赞助商 & 合作伙伴"
                      : "SPONSORS & PARTNERS"}
                  </label>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                    {sponsors.map((sponsor, idx) => (
                      <div
                        key={idx}
                        className="relative bg-gradient-to-br from-brand/5 to-transparent border border-brand/30 p-3 group hover:border-brand/60 hover:shadow-[0_0_15px_rgba(212,163,115,0.15)] transition-all"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-2 bg-gradient-to-br from-black/60 to-brand/5 p-4 border border-brand/30 backdrop-blur-sm">
                    <div className="col-span-1 md:col-span-2">
                      <div className="relative border-2 border-dashed border-brand/40 h-12 w-full bg-black/40 backdrop-blur-sm flex items-center justify-center hover:border-brand/80 hover:bg-brand/5 transition-all cursor-pointer group">
                        {newSponsor.logo ? (
                          ...
                          <span className="text-brand/60 text-[10px] group-hover:text-brand transition-colors">
                            LOGO
                          </span>
                        )}
                        <input
                    ...
                    <button
                      onClick={addSponsor}
                      className="col-span-1 md:col-span-1 bg-brand text-black font-black hover:bg-white hover:scale-105 transition-all"
                    >
                      +
                    </button>
                  </div>
