...
interface CreateHackathonModalProps {
  ...
  lang: "zh" | "en";
  /** When true, renders as a full page (no overlay) with a back button instead of close. Used for /create route. */
  asPage?: boolean;
}
...
export default function CreateHackathonModal({
  ...
  const [activeTab, setActiveTab] = useState<
    "basic" | "timeline" | "details" | "judging"
  >("basic");
  ...
  const pageTitle = initialData
    ...
    ? "发起行动"
    : "INITIATE ACTION";
  const pageSubtitle = initialData
    ? lang === "zh"
      ? "更新活动配置与时间线。"
      : "Update event config and timeline."
    : lang === "zh"
    ? "创建新的黑客松活动，配置时间线与评审规则。"
    : "Create a new hackathon and configure timeline and judging.";
  ...
  // Shared form content (used in both page and modal)
  const formContentEl = (
    <>
      {activeTab === "basic" && (
        <div className="mb-8 p-1 bg-gradient-to-r from-brand/20 to-purple-600/20 border border-brand/30 relative overflow-hidden group">
          ...
          <div className="p-4 bg-black/40 backdrop-blur-sm relative z-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-black text-brand tracking-widest uppercase">
                {lang === "zh" ? "AI 架构师模式" : "AI ARCHITECT MODE"}
              </h3>
            </div>
            <div className="flex gap-0">
              <input
                ...
                placeholder={
                  lang === "zh"
                    ? "输入指令 [例如: AI 医疗创新赛]"
                    : "Enter prompt [e.g. AI Health Innovation]"
                }
                value={aiPrompt}
            ...
            <p className="text-[10px] text-brand/60 mt-2 font-mono uppercase tracking-wider">
              &gt;{" "}
              {lang === "zh"
                ? "通过神经网络链接自动生成清单、规则和评分逻辑。"
                : "Auto-generate manifest, rules, and scoring logic via neural link."}
            </p>
          </div>
        </div>
      )}
      ...
      {activeTab === "basic" && (
        <div className="space-y-6">
          <div>
            ...
            <input
              type="text"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 text-ink focus:border-brand focus:outline-none font-bold tracking-wide placeholder-gray-400"
              value={title}
          ...
          <div>
            ...
            <div className="flex gap-2 mb-2">
              <input
                ...
                className="block w-full text-xs text-gray-400 font-mono file:mr-4 file:py-2 file:px-4 file:border border-brand/20 file:text-xs file:font-mono file:bg-brand/10 file:text-brand hover:file:bg-brand/20 cursor-pointer"
              />
            </div>
            <input
              type="text"
              className="w-full px-4 py-2 bg-white/5 border border-white/10 text-ink focus:border-brand focus:outline-none font-mono text-sm placeholder-gray-400"
              value={coverImage}
            ...
            {coverImage && (
              <img
                src={coverImage}
                alt="Cover Preview"
                className="mt-4 h-40 w-full object-cover border border-white/10 opacity-80"
              />
            )}
          </div>
          <div>
            ...
            <textarea
              className="w-full px-4 py-3 bg-white/5 border border-white/10 text-ink focus:border-brand focus:outline-none font-light h-24 placeholder-gray-400 resize-none"
              value={description}
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                type="text"
                className="w-full px-4 py-2 bg-white/5 border border-white/10 text-ink focus:border-brand focus:outline-none font-mono text-sm placeholder-gray-400"
                value={professionalismTags}
                onChange={(e) => setProfessionalismTags(e.target.value)}
                placeholder="初级, 进阶, 专家..."
              />
            </div>
          </div>
        </div>
      ...
      {activeTab === "timeline" && (
        <div className="space-y-8">
          <div className="p-6 bg-white/5 border border-brand/20 relative">
            <div className="absolute top-0 right-0 p-1 bg-brand text-black text-[10px] font-bold uppercase">
              {lang === "zh" ? "关键路径" : "CRITICAL PATH"}
            </div>
            <h3 className="font-bold text-ink mb-4 uppercase tracking-widest text-sm">
              {lang === "zh" ? "主流程 *" : "MAIN FLOW *"}
            </h3>
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <input
                  type="datetime-local"
                  className="w-full bg-black/50 border border-white/10 text-ink p-2 font-mono text-sm focus:border-brand outline-none"
                  value={startDate}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="datetime-local"
                  className="w-full bg-white/5 border border-white/10 text-ink p-2 font-mono text-xs focus:border-brand outline-none"
        ...
        <div className="space-y-6">
          <div>
            <h3 className="font-bold text-ink text-sm uppercase tracking-widest border-l-2 border-brand pl-3 mb-4">
              {lang === "zh" ? "任命评审" : "APPOINT ARBITERS"}
            </h3>
            <div className="flex gap-2 mb-4">
              <input
                type="email"
                className="flex-1 px-4 py-2 bg-white/5 border border-white/10 text-ink focus:border-brand outline-none font-mono text-sm"
                placeholder={lang === "zh" ? "评审邮箱..." : "Arbiter Email..."}
                value={newJudgeEmail}
              ...
              <button
                type="button"
                onClick={addJudge}
                className="px-4 py-2 bg-white/10 hover:bg-white/20 text-ink font-mono text-xs uppercase border border-white/10"
              >
                {lang === "zh" ? "添加" : "ADD"}
              </button>
            </div>
            <div className="flex flex-wrap gap-2">
              {judges.map((judge) => (
                <span
                  key={judge}
                  className="px-3 py-1 bg-brand/10 border border-brand/30 text-brand text-xs font-mono flex items-center gap-2"
                >
                  {judge}
                  <button
                    type="button"
          ...
          <div className="pt-6 divider-t">
            ...
            <div className="grid grid-cols-12 gap-2 mb-4">
              <div className="col-span-3">
                <input
                  type="text"
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 text-ink font-mono text-xs outline-none focus:border-brand"
                  placeholder={lang === "zh" ? "维度名称" : "Metric Name"}
              ...
              <div className="col-span-1">
                <button
                  ...
                  className="w-full h-full bg-brand/20 hover:bg-brand/30 text-brand font-bold flex items-center justify-center border border-brand/30"
                >
                  +
                </button>
              </div>
            ...
            <div className="space-y-2">
              {scoringDimensions.map((dim, idx) => (
                <div
                  key={idx}
                  className="grid grid-cols-12 gap-2 text-xs font-mono p-2 bg-white/5 border border-white/5"
                >
                  <div className="col-span-3 font-bold text-ink">
                    {dim.name}
                  </div>
      ...
      {activeTab === "details" && (
        <div className="space-y-6">
          <div>
            ...
            <textarea
              className="w-full px-4 py-3 bg-white/5 border border-white/10 text-ink focus:border-brand focus:outline-none font-mono text-xs h-40 placeholder-gray-400 resize-none"
              value={rulesDetail}
              onChange={(e) => setRulesDetail(e.target.value)}
              placeholder="# 活动规则细则..."
            />
          </div>
          <div>
            ...
            <textarea
              className="w-full px-4 py-3 bg-white/5 border border-white/10 text-ink focus:border-brand focus:outline-none font-mono text-xs h-32 placeholder-gray-400 resize-none"
              value={awardsDetail}
              onChange={(e) => setAwardsDetail(e.target.value)}
              placeholder="一等奖: 1000 积分..."
            />
          </div>
        </div>
      )}
  ...
  const footerEl = (
    <div
      className={`border-t flex justify-end space-x-4 ${
        asPage ? "py-6 border-white/10" : "p-6 border-brand/20 bg-surface/50"
      }`}
    >
      <button
        type="button"
        ...
        disabled={loading}
        className="px-6 py-2 border border-white/10 text-gray-400 font-mono text-sm hover:bg-white/5 hover:text-white transition uppercase"
      >
      ...
      <button
        ...
        disabled={loading}
        className="px-8 py-2 bg-brand text-black font-bold font-mono text-sm hover:bg-white hover:text-black transition shadow-[0_0_15px_rgba(212,163,115,0.3)] uppercase"
      >
        {loading
          ? lang === "zh"
            ? "发布中..."
  ...
  // Page layout: native full page (no card), same style as Explore page.
  if (asPage) {
    return (
        ...
        <div className="max-w-[1200px] mx-auto w-full">
          <div className="mb-8">
            <h1 className="text-4xl md:text-5xl font-black text-ink tracking-tighter">
              <span className="text-brand font-mono">//</span> {pageTitle}
            </h1>
          ...
          {tabsEl}
          <div className="py-8 overflow-y-auto flex-1 scrollbar-thin scrollbar-thumb-brand/20 scrollbar-track-transparent">
            {formContentEl}
          </div>
          {footerEl}
        </div>
